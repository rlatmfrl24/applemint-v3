...
export const YoutubeThreads = ({ isActive }: { isActive: boolean }) => {
	const fetchYoutubeThreads = useCallback(async ({ pageParam }: { pageParam?: string }) => {
	...
	const youtubeThreads = useMemo(() => data?.pages.flatMap((page) => page.items) ?? [], [data]);
	...
	if (error) {
		return (
			<Alert variant="destructive" className="mb-4 w-full">
				...
				<AlertDescription>유튜브 스레드를 불러오는 중 오류가 발생했습니다.</AlertDescription>
			</Alert>
		);
	}
