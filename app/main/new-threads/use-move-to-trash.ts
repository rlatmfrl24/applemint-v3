...
export const useMoveThreadToTrash = (thread: ThreadItemType) => {
	...
	return useMutation<string, unknown, string, MutationContext>({
		mutationFn: async (threadId) => {
			...
			const deleteIdentifier = Number.isNaN(parsedId) ? threadId : parsedId;
		...
		onMutate: async (threadId) => {
			...
			const previousStates = affectedQueries.map((query) => {
				const data = query.state.data as InfiniteData<ThreadsPage> | undefined;
				if (data) {
					queryClient.setQueryData(query.queryKey, removeThreadFromInfiniteData(data, threadId));
				}
