...
export const MediaThreads = ({ isActive }: { isActive: boolean }) => {
	const fetchMediaThreads = useCallback(async ({ pageParam }: { pageParam?: string }) => {
	...
	const mediaThreads = useMemo(() => data?.pages.flatMap((page) => page.items) ?? [], [data]);
	...
	if (error) {
		return (
			<Alert variant="destructive" className="mb-4 w-full">
				...
				<AlertDescription>미디어 스레드를 불러오는 중 오류가 발생했습니다.</AlertDescription>
			</Alert>
		);
	}
