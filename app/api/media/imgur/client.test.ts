...
describe("fetchImgurMetadata", () => {
	...
	it("gallery image의 video 종류와 description fallback을 정규화한다", async () => {
		...
		const result = await fetchImgurMetadata(normalizeImgurUrl("https://imgur.com/gallery/Gal123"), {
			clientId: "fixture-client-id",
			fetchImpl: fetchMock,
		});
	...
	it("빈 gallery album은 포함된 images를 재사용해 0개로 정규화한다", async () => {
		...
		const result = await fetchImgurMetadata(
	...
	it("빈 album URL은 album images 응답을 확인한 뒤 0개로 정규화한다", async () => {
		...
		const result = await fetchImgurMetadata(normalizeImgurUrl("https://imgur.com/a/Empty12"), {
			clientId: "fixture-client-id",
			fetchImpl: fetchMock,
		});
