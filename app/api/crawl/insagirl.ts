  ...
  try {
    await Promise.all(
      target.map(async (url, index) => {
        console.log(
          `[Insagirl] URL ${index + 1}/${target.length} 크롤링 시작: ${url}`,
        );
        ...
        try {
          ...
          console.log(
            `[Insagirl] URL ${index + 1} JSON 파싱 완료, 원본 아이템 개수: ${
              json.v?.length || 0
            }`,
          );
          ...
          if (!json.v || !Array.isArray(json.v)) {
            console.warn(
              `[Insagirl] URL ${index + 1} 예상과 다른 JSON 구조:`,
              json,
            );
            return;
          }
          ...
          console.log(
            `[Insagirl] URL ${
              index + 1
            } 필터링 후 아이템 개수: ${filteredItems.length}`,
          );
          ...
          console.error(
            `[Insagirl] URL ${index + 1} 크롤링 중 에러 발생:`,
            urlError,
          );
    ...
    console.log(
      `[Insagirl] 전체 수집된 아이템 개수 (중복 포함): ${list.length}`,
    );
    ...
    console.log(
      `[Insagirl] 중복 제거 후 최종 아이템 개수: ${uniqueList.length}`,
    );
