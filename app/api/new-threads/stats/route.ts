...
export async function GET(request: NextRequest) {
	try {
		...
		if (scope === "normal") {
			query.neq("type", MEDIA_SCOPE).neq("type", YOUTUBE_SCOPE);
		}
		...
		const counts = rows.reduce(
			(
				acc: Record<string, { key: string; label: string; count: number }>,
				row: { type: string; tag: string[] | null }
			) => {
				const isIssuelink = row.type === ISSUELINK_SCOPE;
				const key = isIssuelink ? `${ISSUELINK_SCOPE}::${row.tag?.[1] ?? "unknown"}` : row.type;
				const label = isIssuelink ? (row.tag?.[1] ?? "unknown") : row.type;
