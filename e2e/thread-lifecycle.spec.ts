...
test("Main에서 Quick과 Trash를 거쳐 스레드를 복원한다", async ({ page }) => {
	const [thread] = await seedThreads("new-threads", 1, { prefix: "lifecycle" });
	...
	await expect.poll(() => countThreads("quick-save")).toBe(1);
	...
	await expect(card()).toHaveCount(0);
	await expect.poll(() => countThreads("new-threads")).toBe(1);
