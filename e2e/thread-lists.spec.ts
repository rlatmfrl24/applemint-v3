...
import { clearThreadTables, type E2EThreadTable, seedThreads } from "./support/database";

const listCases: Array<{ table: E2EThreadTable; path: string }> = [
	{ table: "new-threads", path: "/main" },
	{ table: "quick-save", path: "/main/quick" },
	{ table: "trash", path: "/main/trash" },
];
...
for (const { table, path } of listCases) {
	test(`${table} 목록을 복합 커서로 끝까지 불러온다`, async ({ page }) => {
		await seedThreads(table, 31, { prefix: `${table}-pagination` });
		const firstPageResponse = page.waitForResponse((response) => {
			const url = new URL(response.url());
			return url.pathname === `/api/${table}` && !url.searchParams.has("cursor");
		});
		...
		await page.getByTestId(`${table}-load-more-sentinel`).scrollIntoViewIfNeeded();
...
test("Main 필터 변경 시 다른 목록 cache가 섞이지 않는다", async ({ page }) => {
	await seedThreads("new-threads", 26, { prefix: "filter-arcalive", type: "arcalive" });
	await seedThreads("new-threads", 5, { prefix: "filter-battlepage", type: "battlepage" });
	...
	const filteredRequest = page.waitForRequest((request) => {
		...
		return (
			url.pathname === "/api/new-threads" && url.searchParams.get("filterType") === "battlepage"
		);
	});
