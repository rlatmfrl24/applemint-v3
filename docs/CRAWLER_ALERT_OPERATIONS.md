...
Applemint는 Supabase Cron에서 `crawl_runs`를 15분 간격으로 평가하고, 소스별 장애 에피소드
하나당 GitHub Issue 하나를 유지합니다. 같은 장애가 지속되면 24시간 후 댓글로 다시 알리고,
모든 신호가 정상화되면 복구 댓글과 함께 Issue를 종료합니다.
...
- Variable `APP_BASE_URL`: 운영 Next 애플리케이션 origin
- Secret `CRAWL_INTERNAL_SECRET`: Next와 동일한 32바이트 이상 내부 secret

Issue 생성에는 별도 PAT를 사용하지 않고 workflow의 `GITHUB_TOKEN`과 `issues: write` 최소 권한만
사용합니다. GitHub에는 Supabase service role key를 제공하지 않습니다. workflow는 제한된
`POST /api/crawl/alerts/notifications`만 호출하며 DB alert RPC는 Next 서버 내부에서 실행됩니다.

Supabase의 `applemint-evaluate-crawl-alerts` Cron은 매시 5·20·35·50분에 장애를 평가합니다.
`.github/workflows/crawler-health.yml`은 2분 뒤인 7·22·37·52분에 outbox를 GitHub Issue로 전달합니다.
GitHub Actions 예약 실행은 부하에 따라 지연될 수 있으므로 15분은 목표 전달 주기입니다.

## 배포와 첫 확인

1. `supabase db push`로 alert migration을 먼저 적용합니다.
2. Next 애플리케이션을 배포합니다.
3. GitHub의 `APP_BASE_URL`, `CRAWL_INTERNAL_SECRET`을 등록합니다.
...
- 내부 알림 API 조회 자체가 실패하면 `[Crawler Monitor] 상태 확인 실패` Issue 하나가 유지되며 연결 복구 후 자동 종료됩니다.
