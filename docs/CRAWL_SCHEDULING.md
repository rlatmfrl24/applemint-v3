...
- 활성 크롤링 소스의 초기 최소 실행 간격은 `arcalive` 2시간, `battlepage` 4시간,
  `dogdrip` 3시간, `insagirl` 3시간, `issuelink` 3시간입니다.
...
- DogDrip은 robots.txt의 `Crawl-delay: 10`보다 충분히 긴 3시간 간격으로 인기글 목록 첫
  페이지만 한 번 요청합니다. 게시물 본문·이미지는 요청하지 않고 제목과 직접 링크만 저장하며,
  같은 실행에서 실패 요청을 재시도하지 않습니다. 수동 실행에도 DB admission에서 최소 10초
  간격을 강제합니다.
