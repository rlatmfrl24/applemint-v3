...
404·410은 해당 기기 구독만 비활성화하고, 그 외 재시도할 수 없는 4xx는 즉시 `dead`로
종료합니다. 각 Push 요청은 15초 안에 응답하지 않으면 네트워크 실패로 분류합니다. 브라우저가
보고한 구독 만료 시각을 지난 기기와 미발송 delivery는 다음 dispatcher 점검에서 각각 비활성화와
`skipped`로 정리합니다.
...
| `재연결 필요` | 브라우저 구독은 남아 있지만 서버 구독이 비활성화됐는지 | `알림 활성화`를 눌러 기존 구독을 교체하고 서버에 다시 등록 |
